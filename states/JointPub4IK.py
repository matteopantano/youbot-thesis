...
class JointPubIK(EventState):
	'''
	Publish Joint Values.

	># goal          object     Target for IK.
	...
	def __init__(self):
		...
		Constructor
		'''
		super(JointPubIK, self).__init__(outcomes=['done', 'failed'], input_keys = ['goal'])
	...
	def execute(self, userdata):
		...
		if (abs(self._joint_positions[0] - self.joint_states.position[10])) <= 0.015:
			self.done = self.done + 1

		if (abs(self._joint_positions[1] - self.joint_states.position[11])) <= 0.015:
			self.done = self.done + 1

		if (abs(self._joint_positions[2] - self.joint_states.position[12])) <= 0.015:
			self.done = self.done + 1

		if (abs(self._joint_positions[3] - self.joint_states.position[13])) <= 0.015:
			self.done = self.done + 1

		if (abs(self._joint_positions[4] - self.joint_states.position[14])) <= 0.015:
			self.done = self.done + 1
	...
	def on_enter(self, userdata):
		...
		self._joint_positions = userdata.goal
