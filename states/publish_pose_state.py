...
class PublishPoseState(EventState):
	...
	def __init__(self, topic, theta):
		"""Constructor"""
		super(PublishPoseState, self).__init__(outcomes=['received', 'unavailable'],output_keys=['output_value','message'])
	...
	def execute(self, userdata):
		...
		if self._sub.has_msg(self._topic):
			...
			print 'dataposition: ', data.position.x
			# go to pose of cube
			target_pose = Pose2D(data.position.x - 0.4*math.cos(self._theta), data.position.y - 0.4*math.sin(self._theta), self._theta) #mettere teta come parametro esterno, da sopra
			userdata.output_value = target_pose
			self._sub.remove_last_msg(self._topic)
			Logger.logwarn('received')
			return 'received'
	...
	def on_exit(self, userdata):
		Logger.loginfo('Exiting Pub Pose State')
