...
class GripperStateClosed(EventState):
	'''
	Close the gripper.
	...
	def __init__(self, duration = 1.0):
		...
		Constructor
		'''
		super(GripperStateClosed, self).__init__(outcomes=['done', 'failed'])
	...
	def execute(self, userdata):
		...
		current_value_l = abs(0.0 - self.joint_states.position[15])	
		current_value_r = abs(0.0 - self.joint_states.position[16])		
	...
	def on_exit(self, userdata):
		Logger.loginfo('Exiting Gripper State Closed')
