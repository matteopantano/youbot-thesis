...
class arm_kinematics:
	...
	def kinematics(self, pose):
		...
		pose_x = pose.position.x * 1000
		pose_y = pose.position.y * 1000
		pose_z = pose.position.z * 1000 # + quantity to stay above the cube
		print(pose_x, pose_y, pose_z)

		b = 176
		c = 135
		d = 155
		e = 230 - pose_z # + 30 per altezza cubo 
		link_12_xy = 33 # vector btw arm_link_1 and arm_link_2
		
		gamma = 1.57
		
		teta_5 = 2.75   #math.asin(-2*(pose_orientation_x*pose_orientation_z -pose_orientation_w*pose_orientation_y)) 
		...
		self.value = (joint_1, joint_2, joint_3, joint_4, joint_5)		
		print("_____________________ joint values")	
		print(self.value)
